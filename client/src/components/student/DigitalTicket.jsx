...
import { generateQRApi } from '../../api/qr.api';

/**
 * DigitalTicket — renders a styled ticket with a real scannable QR code.
 * Props:
 *   ticket: { registrationId, specialId, studentName, eventName, eventDate,
 *             venue, fee, paymentStatus, teamName?, teamMembers?[] }
 *   onClose: () => void
 */
export default function DigitalTicket({ ticket, onClose }) {
    ...
    useEffect(() => {
        if (!ticket.registrationId) { setQrLoading(false); return; }
        generateQRApi(ticket.registrationId)
            .then((r) => setQrDataUrl(r.data.data.qrDataUrl))
            ...
            <div className="w-full max-w-lg space-y-4 my-4">
                ...
                >
                    ...
                    <div className="px-6 py-5 flex items-center justify-between gap-4">
                        <div className="w-24 h-24 bg-gray-50 rounded-xl flex items-center justify-center shrink-0 border border-gray-200 overflow-hidden">
                            {qrLoading ? (
                                <div className="w-6 h-6 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin" />
                            ) : qrDataUrl ? (
                                <img src={qrDataUrl} alt="QR Code" className="w-full h-full object-contain" />
                            ) : (
                                <QRFallback value={ticket.specialId} />
                            )}
                        </div>
...
/** SVG fallback if QR fetch fails */
    ...
    return (
        <svg viewBox="0 0 70 70" width="80" height="80" xmlns="http://www.w3.org/2000/svg">
            ...
            {cells.map((on, i) => {
                ...
                return on ? <rect key={i} x={22 + col * 7} y={22 + row * 7} width={5} height={5} rx="0.5" fill="#1e1b4b" /> : null;
            })}
        </svg>
    );
}
